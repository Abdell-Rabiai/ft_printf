...
/*   Updated: 2022/11/14 14:15:51 by arabiai          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ft_put_adress_count(void *adresse, int fd, int *count, char c)
{
	size_t nb = (size_t)adresse;
	ft_putstr_fd_count("0x", fd, count);
	ft_putnbr_fd_hexa_count(nb, fd, count, c);
}
...
void ft_convert_format(va_list arg_p, char format_character, int *count)
{
	if(format_character == '%')
		...
		ft_put_adress_count(va_arg(arg_p, void *), 1, count, format_character);
}
